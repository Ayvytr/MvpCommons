...
public class NumberPickerView extends View {
    ...
    private int mMinShowIndex = 0;
    private int mMaxShowIndex = 99;
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            if(attr == R.styleable.NumberPickerView_npv_ShowCount) {
                ...
                mTextSizeHint = a
                        .getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_HINT_SP));
            } else if(attr == R.styleable.NumberPickerView_npv_MinValue) {
                int min = a.getInteger(attr, 0);
                if(min < 0) {
                    throw new IllegalArgumentException("Min value must >= 0");
                }
                mMinValue = min;
            } else if(attr == R.styleable.NumberPickerView_npv_MaxValue) {
                int max = a.getInteger(attr, 0);
                setMaxValue(max);

            } else if(attr == R.styleable.NumberPickerView_npv_MinShowIndex) {
                mMinShowIndex = a.getInteger(attr, 0);
                if(mMinShowIndex > mMaxValue - mMinValue) {
                    mMinShowIndex = 0;
                }
            } else if(attr == R.styleable.NumberPickerView_npv_MaxShowIndex) {
                mMaxShowIndex = a.getInteger(attr, 99);
                if(mMaxShowIndex > mMaxValue - mMinValue) {
                    mMaxShowIndex = mMaxValue - mMinValue;
                }
            } else if(attr == R.styleable.NumberPickerView_displayValue) {
                initDisplayClass(a.getString(attr));
                ...
                mItemPaddingHorizontal = a
                        .getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_H));
...
//                mAlterTextArrayWithoutMeasureHint = a.getTextArray(attr);
                ...
                mRespondChangeInMainThread = a
                        .getBoolean(attr, DEFAULT_RESPOND_CHANGE_IN_MAIN_THREAD);
    ...
    private void initDisplayClass(String displayValueClassName) {
        if(displayValueClassName == null) {
            display = new DefaultNumberDisplayImpl();
        } else {
            String className = getFullClassName(getContext(), displayValueClassName);

            try {
                Class<? extends Display> c = Class.forName(className).asSubclass(
                        Display.class);
                display = (Display) c.newInstance();
            } catch(Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private String getFullClassName(Context context, String className) {
        ...
        return NumberPickerView.class.getPackage().getName() + '.' + className;
    }


//    private CharSequence[] getDefaultDisplayedIntRangeValues() {
//        int size = getValueCount(mMaxValue, mMinValue, 1);
//        CharSequence[] cs = new CharSequence[size];
//        for(int i = 0; i < size; i++) {
//            cs[i] = String.valueOf(mMinValue + i);
//        }
//        return cs;
//    }

    private int getValueCount() {
        return display.getRealValueCount(mMinValue, mMaxValue);
    }
    ...
    private void init(Context context) {
        ...
        if(display == null) {
            initDisplayClass(null);
        }
//        if(mDisplayedValues == null) {
//            mDisplayedValues = getDefaultDisplayedIntRangeValues();
//        }
//        mMaxShowIndex = mDisplayedValues.length - 1;
//        mMaxValue = mMaxShowIndex;
        int maxIndex = mMaxValue - mMinValue;
        if(mMaxShowIndex > maxIndex) {
            mMaxShowIndex = maxIndex;
        }
...
//    }
//
    public CharSequence getCurrentDisplayedValue() {
            ...
            try {
                ...
                if(mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw
                            .onValueChangeRelativeToRaw(NumberPickerView.this, mPrevPickedIndex,
                                    currPickedIndex
//                                    ,
//                                    mDisplayedValues
                            );
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange) {
        ...
        if(oldVal != newVal) {
            if(respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                ...
                if(mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw
                            .onValueChangeRelativeToRaw(NumberPickerView.this, oldVal, newVal
//                                    , mDisplayedValues
                            );
                }
            }
...
//        mMaxValue = maxValue;
    ...
    public void setValueRange(int minValue, int maxValue) {
        setValueRange(minValue, maxValue, null);
    }

    private void setValueRange(int minValue, int maxValue, Class<Display> displayClass) {
        stopRefreshing();
...
//        } else {
        if(minShowIndex < 0 || maxShowIndex < 0) {
            throw new IllegalArgumentException("minShowIndex and maxShowIndex must >= 0");
        }  else{
            int maxIndex = mMaxValue - mMinValue;
            if(minShowIndex > maxIndex || maxShowIndex > maxIndex) {
                throw new IllegalArgumentException(
                        "minShowIndex and maxShowIndex must < content count");
            }
        }

        if(minShowIndex > maxShowIndex) {
            throw new IllegalArgumentException("maxShowIndex must >= minShowIndex");
        }
...
//        if(mDisplayedValues != null) {
    ...
    private int measureWidth(int measureSpec) {
        ...
        if(specMode == MeasureSpec.EXACTLY) {
            ...
            int marginOfHint = Math
                    .max(mWidthOfHintText, mWidthOfAlterHint) == 0 ? 0 : mMarginEndOfHint;
            int gapOfHint = Math
                    .max(mWidthOfHintText, mWidthOfAlterHint) == 0 ? 0 : mMarginStartOfHint;

            int maxWidth =
//                    Math.max(mMaxWidthOfAlterArrayWithMeasureHint,
//                    Math.max(mMaxWidthOfDisplayedValues, mMaxWidthOfAlterArrayWithoutMeasureHint)
                    mMaxWidthOfDisplayedValues
                            + 2 * (gapOfHint + Math
                            .max(mWidthOfHintText,
                                    mWidthOfAlterHint) + marginOfHint + 2 * mItemPaddingHorizontal)
//            )
                    ;
            result = this.getPaddingLeft() + this
                    .getPaddingRight() + maxWidth;//MeasureSpec.UNSPECIFIED
...
//                CharSequence str = mDisplayedValues[index + mMinShowIndex];
                CharSequence str = display.getDisplayValue(index + mMinShowIndex,
                        mMinValue, mMaxValue);
    ...
    private void updateMaxWidthOfDisplayedValues() {
        ...
        mPaintText.setTextSize(mTextSizeSelected);
        mMaxWidthOfDisplayedValues = display.getMaxTextWidth(mPaintText, mMinValue, mMaxValue);
...
//    }
    ...
    private void updateWrapStateByContent() {
        if(mDisableWrapSelectorWheelCheck) {
            mWrapSelectorWheelCheck = getValueCount() < mShowCount ? false : true;
        } else {
...
//                CharSequence[] displayedValues
    ...
    public static abstract class Display {
        protected abstract CharSequence getDisplayValue(int i, int minValue, int maxValue);

        public abstract int getRealValueCount(int mMinValue, int mMaxValue);

        public abstract int getMaxTextWidth(TextPaint mPaintText, int mMaxValue, int mMinValue);
    }
}