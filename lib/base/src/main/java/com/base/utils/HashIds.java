...
public class HashIds {
    ...
    private static final String ACCOUNT_SALT_VALUE = "20190415";
    private static final String ACCOUNT_ALPHABET_VALUE = "1234567890";
    private static final int ACCOUNT_MINHASHLENTH_VALUE = 6;
    ...
    private static HashIds mInstance;

    public static HashIds getInstance(){
        if (mInstance == null){
            mInstance = new HashIds(ACCOUNT_SALT_VALUE, ACCOUNT_MINHASHLENTH_VALUE, ACCOUNT_ALPHABET_VALUE);
        }

        return mInstance;
    }

    public String getBankId(String ids) {

        if (!TextUtils.isEmpty(ids)) {
            ids = mInstance.encode(new Long(new Long(ids)));
        }
        return ids;
    }
